...
const FpoPrivateFiles = () => {
  ...
  const fetchFiles = useCallback(async type => {
    try {
      ...
      if (!response.ok) {
        console.log(
          `⚠️ Admin endpoint failed with status ${response.status}, trying /api/user/files/private`,
        );
      ...
      if (!contentType || !contentType.includes('application/json')) {
        ...
        throw new Error(
          'Server returned non-JSON response. Check if endpoint exists.',
        );
      }

      const json = await response.json();
      console.log(
        '📂 Distributor Private files response:',
      ...
      if (!response.ok) {
        ...
        console.log(
          `⚠️ API returned error: ${msg}. Attempting fallback to Profile data.`,
        );
  ...
  const handleView = async file => {
    ...
    try {
      ...
      Alert.alert(t('cannot_open_file'), t('device_cant_open'));
  ...
  const renderFile = ({ item }) => {
    ...
    console.log('📅 Distributor Date fields:', {
