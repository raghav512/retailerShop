...
const ActionButton = ({
  ...
  isActive,
  onPress,
  activeColor = FPO_COLORS.primary,
}) => (
  <TouchableOpacity
    onPress={onPress}
    ...
    activeOpacity={0.7}
  >
    <Icon name={icon} size={20} color={isActive ? activeColor : '#555'} />
...
const CommentItem = React.memo(({ comment }) => {
  ...
  console.log('Comment item:', JSON.stringify(comment, null, 2));
  console.log('Comment user:', comment?.user);
  console.log('Comment image:', comment?.avatar);

  const avatarSource = getAvatarSource(comment.avatar);
  ...
  return (
    <View style={styles.commentItem}>
      <Image
        ...
        onLoad={() => console.log('Comment image loaded:', avatarSource?.uri)}
        onError={e =>
          console.log('Comment image failed:', avatarSource?.uri, e.nativeEvent)
        }
      />
  ...
  return (
    ...
    >
      <View style={styles.modalOverlay}>
        ...
        <Animated.View
          style={[styles.commentDrawer, { transform: [{ translateY }] }]}
        >
          {/* Handle */}
      ...
      <View style={styles.cardContent}>
        ...
        <View style={styles.userInfo}>
          <Image
            ...
            onError={e =>
              console.log('Image failed:', avatarSource?.uri, e.nativeEvent)
            }
          />
        ...
        <View style={styles.actions}>
          ...
          <TouchableOpacity
            onPress={() => {
              console.log('Share button pressed', post.id);
...
const FpoAllPosts = () => {
  ...
  useEffect(() => {
    ...
    console.log('Component mounted, loading user data and posts');
    loadUserData();
    fetchPosts();
  }, [fetchPosts]);
  ...
  const loadUserData = async () => {
    try {
      ...
      console.log('Raw userId from storage:', userId);
    ...
    try {
      ...
      if (data.status === 'success' && data.data) {
        const formattedPosts = data.data.map(post => {
          ...
          // Determine avatar - prioritize current user's profile image for their posts
          let postAvatar = null;

          // Check if this is the current user's post (compare both userId and user._id)
          ...
          if (isCurrentUserPost && currentUserProfileImage) {
            // Use current user's profile image from AsyncStorage
            postAvatar = currentUserProfileImage;
          } else if (post.user && post.user.profileImage) {
            // Use post user's profile image
            postAvatar = post.user.profileImage;
          } else {
            // Fallback to post.avatar field
            postAvatar = post.avatar;
          }

          console.log('Post avatar selection:', {
            postId: post._id,
  ...
  const handleAction = async (postId, action) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/posts/${endpointMap[action]}`,
        {
          method: 'POST',
          headers: {
          ...
          prevPosts.map(post => {
            ...
            if (action === 'like') {
              ...
              if (isLiked) {
                ...
                newPost.dislikes = newPost.dislikes.filter(
                  dislike => dislike._id !== userId,
                );
              }
            } else if (action === 'dislike') {
              const isDisliked = newPost.dislikes.some(
                dislike => dislike._id === userId,
              );
              if (isDisliked) {
                newPost.dislikes = newPost.dislikes.filter(
  ...
  const handleShare = async post => {
    try {
      console.log('Calling native share');
      await Share.share({
        message: `Check out this post: ${post.title || 'Interesting post'}`,
        url: post.post_image || '',
      });
    } catch (error) {
      console.log('SHARE FLOW ERROR:', error);
    }
  };
  ...
  const handleSubmitComment = async () => {
    ...
    // Get current user's profile image for the comment
    let userAvatar = null;
    try {
      const userData = await AsyncStorage.getItem('userData');
      if (userData) {
      ...
      console.error('Error loading user avatar for comment:', error);
    }
    ...
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>
          {t('community_screen.all_posts_loading')}
        </Text>
      </View>
    );
  }
      ...
      <FlatList
        ...
        refreshControl={
          <RefreshControl
            ...
            tintColor={FPO_COLORS.primary}
          />
        }
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>
              {t('community_screen.no_posts_available')}
            </Text>
          </View>
        }
      />
