...
const BroadcastsScreen = ({ navigation }) => {
    ...
    >
      <View style={styles.cardHeader}>
        ...
        <View style={styles.headerRight}>
          <Text style={styles.cardDate}>{formatDate(item.sentAt || item.createdAt)}</Text>
        </View>
      </View>
      ...
      <View style={styles.cardFooter}>
        <View style={styles.badge}>
          <Icon name="people" size={12} color={THEME} style={{ marginRight: 4 }} />
          <Text style={styles.badgeText}>
            {item.targetRole === 'all' ? 'For Everyone' : `For ${capitalize(item.targetRole)}s`}
          </Text>
        </View>
  ...
  const renderEmpty = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptyText}>{t("no_broadcasts") || "No broadcasts yet"}</Text>
      <Text style={styles.emptySubText}>
        {t("no_broadcasts_desc") || "You'll see important updates and announcements here"}
      </Text>
    </View>
  );
  ...
  return (
    <View style={styles.safeArea}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{t("broadcasts") || "Broadcasts"}</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={[THEME]} tintColor={THEME} />}
...
const styles = StyleSheet.create({
  ...
  iconBox: { width: 36, height: 36, borderRadius: 12, backgroundColor: '#EBF3F6', justifyContent: 'center', alignItems: 'center' },
  ...
  cardDescription: { fontSize: 14, color: '#6B7280', lineHeight: 22, marginBottom: 16 },
  ...
  badge: { backgroundColor: '#EBF3F6', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16 },
  ...
  emptySubText: { fontSize: 14, color: '#9CA3AF', textAlign: 'center', paddingHorizontal: 40, lineHeight: 20 },
