...
const FarmerListing = () => {
  ...
  const getStatusStyle = (status) => {
    switch (status?.toLowerCase()) {
      ...
      default: return { bg: '#e2f0c9', color: '#F59E0B' };
    }
  };

  const renderItem = ({ item }) => {
    const farmer = `${item.userId?.firstName || ""} ${item.userId?.lastName || ""}`.trim() || t("farmer_listing_details.unknown") || "Unknown Farmer";
    const code = item._id?.slice(-5).toUpperCase();
    const crop = item.cropName;
    const quantity = `${item.quantity} ${t("farmer_listing_details.quintal") || 'Quintals'}`;
      ...
      >
        <View style={styles.cardHeader}>
          ...
          <View style={[styles.badge, { backgroundColor: statusStyle.bg }]}>
            <Text style={[styles.badgeText, { color: statusStyle.color }]}>{item.status || "Pending"}</Text>
          </View>
        </View>
  ...
  return (
    <View style={styles.safeArea}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{t("farmer_listing") || "Farmer Listings"}</Text>
        <View style={{ width: 40 }} />
      </View>
...
const styles = StyleSheet.create({
  ...
  infoGrid: { flexDirection: 'row', backgroundColor: '#FAFAFA', borderRadius: 16, padding: 16, marginBottom: 16, borderWidth: 1, borderColor: '#F3F4F6' },
  infoCol: { flex: 1 },
  label: { fontSize: 11, color: "#6B7280", fontWeight: "600", marginBottom: 4, textTransform: 'uppercase' },
  ...
  farmerName: { fontSize: 13, color: "#4B5563", fontWeight: "600", marginLeft: 6 },
