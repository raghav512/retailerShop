...
const UNITS = [
  { id: '1', key: 'bag', value: 'bag' },
  { id: '2', key: 'packet', value: 'packet' },
  { id: '3', key: 'box', value: 'box' },
  { id: '4', key: 'bottle', value: 'bottle' },
  { id: '5', key: 'can', value: 'can' },
  { id: '6', key: 'ml', value: 'ml' },
  { id: '7', key: 'gm', value: 'gm' },
];

const CATEGORIES = [
  { id: '1', key: 'fertilizers', value: 'fertilizers' },
  { id: '2', key: 'seeds', value: 'seeds' },
  { id: '3', key: 'insecticides', value: 'insecticides' },
  { id: '4', key: 'organic', value: 'organic' },
  { id: '5', key: 'pgr', value: 'pgr' },
  { id: '6', key: 'animal_feed', value: 'animal_feed' },
];
...
const UpdateProduct = () => {
  ...
  const pickImageFromGallery = async () => {
    if (selectedImages.length + form.productImages.length >= 5) {
      showAlert({ type: 'warning', title: t('update_product.limit_reached'), message: t('update_product.max_images') });
      return;
    }
  ...
  const pickVideo = async () => {
    if (selectedVideos.length + form.productVideos.length >= 3) {
      showAlert({ type: 'warning', title: t('update_product.limit_reached'), message: t('update_product.max_videos') });
      return;
    }
    ...
    if (result) {
      ...
      const existingVideosCount = form.productVideos.filter(v => typeof v === 'string').length;
      handleChange('productVideos', [
        ...form.productVideos.slice(0, existingVideosCount),
  ...
  const handleUpdate = async () => {
    ...
    // Merge images: keep existing (by URL) + add new as base64
    ...
    updatePayload.productImages = [...existingImageUrls, ...newBase64Images];

    // Build productVideos: filter out nulls/invalid and merge
    const finalVideos = [...form.productVideos.filter(v => typeof v === 'string')];
    // In a real scenario we'd handle base64 encoding for new videos if needed, 
    // but here we follow the existing pattern in UpdateProduct.js
    updatePayload.productVideos = finalVideos;
    ...
    try {
      ...
      showAlert({ type: 'error', title: t('error'), message: t('update_product.update_failed') });
    } finally {
      setLoading(false);
    }
  };

  const existingVideos = form.productVideos.filter(v => typeof v === 'string');
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.sectionCard}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.sectionTitle}>{t('update_product.product_images')} ({selectedImages.length + form.productImages.length}/5)</Text>
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.imageScroll}>
            {form.productImages.map((img, index) => (
              <View key={`existing-${index}`} style={styles.imageWrapper}>
                <Image source={{ uri: img.url || img }} style={styles.productImage} />
                ...
                <View style={styles.existingBadgeContainer}>
                  <Text style={styles.existingBadge}>{t('update_product.existing')}</Text>
                </View>
              </View>
            ))}
            ...
            {(selectedImages.length + form.productImages.length) < 5 && (
              <TouchableOpacity onPress={() => setShowImagePicker(true)} style={styles.addImageBtn} activeOpacity={0.7}>
                ...
                <Text style={styles.imagePlaceholderText}>{t('update_product.add')}</Text>
              </TouchableOpacity>
            )}
          </ScrollView>
          ...
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.sectionTitle}>{t('update_product.product_videos')} ({existingVideos.length + selectedVideos.length}/3)</Text>
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.imageScroll}>
            {existingVideos.map((uri, index) => (
              <View key={`ev-${index}`} style={styles.videoThumb}>
                <Icon name="videocam" size={30} color={THEME} />
                <Text style={styles.videoName} numberOfLines={1}>{t('update_product.video_number', { number: index + 1 })}</Text>
                <TouchableOpacity style={styles.removeImageBtn} onPress={() => removeExistingVideo(index)}>
                  <Icon name="close-circle" size={24} color="#EF4444" />
                </TouchableOpacity>
                <View style={styles.existingBadgeContainer}>
                  <Text style={styles.existingBadge}>{t('update_product.existing')}</Text>
            ...
            {selectedVideos.map((vid, index) => (
              <View key={`nv-${index}`} style={styles.videoThumb}>
                <Icon name="videocam" size={30} color={THEME} />
                <Text style={styles.videoName} numberOfLines={1}>{vid.fileName || `${t('update_product.video')} ${existingVideos.length + index + 1}`}</Text>
                <TouchableOpacity style={styles.removeImageBtn} onPress={() => removeNewVideo(index)}>
                  <Icon name="close-circle" size={24} color="#EF4444" />
                </TouchableOpacity>
              </View>
            ))}
            {(existingVideos.length + selectedVideos.length) < 3 && (
              <TouchableOpacity style={styles.addImageBtn} onPress={pickVideo} activeOpacity={0.8}>
                ...
                <Text style={styles.imagePlaceholderText}>{t('update_product.add_video')}</Text>
              </TouchableOpacity>
            )}
          </ScrollView>
        </View>
        ...
        <View style={styles.sectionCard}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.sectionTitle}>{t('update_product.product_info')}</Text>
          </View>

          <Text style={styles.label}>{t('update_product.product_name_required')}</Text>
          <TextInput style={styles.input} placeholder={t('update_product.product_name_ph')} placeholderTextColor="#9CA3AF" value={form.productName} onChangeText={v => handleChange('productName', v)} />

          <Text style={styles.label}>{t('update_product.brand_required')}</Text>
          <TextInput style={styles.input} placeholder={t('update_product.brand_ph')} placeholderTextColor="#9CA3AF" value={form.brand} onChangeText={v => handleChange('brand', v)} />

          <Text style={styles.label}>{t('update_product.description')}</Text>
          <TextInput style={styles.textArea} placeholder={t('update_product.description_ph')} placeholderTextColor="#9CA3AF" multiline numberOfLines={4} value={form.description} onChangeText={v => handleChange('description', v)} />
        </View>
        ...
        <View style={styles.sectionCard}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.sectionTitle}>{t('update_product.category_crops')}</Text>
          </View>

          <Text style={styles.label}>{t('update_product.product_category_required')}</Text>
          <TouchableOpacity style={styles.select} onPress={() => setShowCategoryDropdown(!showCategoryDropdown)}>
            <Text style={[styles.selectText, form.productCategory && { color: '#1F2937' }]}>
              {form.productCategory ? t(`product_categories.${CATEGORIES.find(c => c.value === form.productCategory)?.key}`) : t('update_product.select_category')}
            </Text>
            ...
            <View style={styles.dropdown}>
              {CATEGORIES.map(cat => (
                <TouchableOpacity key={cat.id} style={styles.option} onPress={() => { handleChange('productCategory', cat.value); setShowCategoryDropdown(false); }}>
                  <Text style={styles.optionText}>{t(`product_categories.${cat.key}`)}</Text>
                </TouchableOpacity>
              ))}
            </View>
          )}

          <Text style={styles.label}>{t('update_product.target_crops')}</Text>
          <TextInput style={styles.input} placeholder={t('update_product.target_crops_ph')} placeholderTextColor="#9CA3AF" value={form.targetCrops} onChangeText={v => handleChange('targetCrops', v)} />
        </View>
        ...
        <View style={styles.sectionCard}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.sectionTitle}>{t('update_product.technical_details')}</Text>
          </View>

          <Text style={styles.label}>{t('update_product.technical_details')}</Text>
          <TextInput style={styles.textArea} placeholder={t('update_product.technical_details_ph')} placeholderTextColor="#9CA3AF" multiline numberOfLines={3} value={form.productTechnicalDetails} onChangeText={v => handleChange('productTechnicalDetails', v)} />

          <Text style={styles.label}>{t('update_product.how_to_use')}</Text>
          <TextInput style={styles.textArea} placeholder={t('update_product.how_to_use_ph')} placeholderTextColor="#9CA3AF" multiline numberOfLines={3} value={form.howToUse} onChangeText={v => handleChange('howToUse', v)} />

          <Text style={styles.label}>{t('update_product.product_benefits')}</Text>
          <TextInput style={styles.textArea} placeholder={t('update_product.product_benefits_ph')} placeholderTextColor="#9CA3AF" multiline numberOfLines={3} value={form.productBenefits} onChangeText={v => handleChange('productBenefits', v)} />
        </View>
        ...
        <View style={styles.sectionCard}>
          <View style={[styles.sectionHeader, { justifyContent: 'space-between' }]}>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              <Text style={styles.sectionTitle}>{t('update_product.variants')}</Text>
            </View>
            <TouchableOpacity onPress={addVariant} style={styles.addVariantBtn} activeOpacity={0.8}>
              <Icon name="add" size={18} color={THEME} />
              <Text style={styles.addVariantText}>{t('update_product.add') || 'Add'}</Text>
            </TouchableOpacity>
          </View>
          ...
          {variants.map((variant, index) => (
            <View key={index} style={styles.variantCard}>
              <View style={styles.variantCardHeader}>
                <Text style={styles.variantLabel}>{t('update_product.variant_number', { number: index + 1 })}</Text>
                {variants.length > 1 && (
              ...
              <View style={styles.row}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.label}>{t('update_product.sku')}</Text>
                  <TextInput style={styles.input} placeholder={t('update_product.sku_ph')} placeholderTextColor="#9CA3AF" keyboardType="numeric" value={variant.parameter} onChangeText={v => updateVariant(index, 'parameter', v)} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.label}>{t('update_product.unit_required')}</Text>
                  <TouchableOpacity style={styles.select} onPress={() => setShowUnit(showUnit === index ? null : index)}>
                    <Text style={styles.selectText}>{variant.unit ? t(`product_units.${UNITS.find(u => u.value === variant.unit)?.key}`) : t('update_product.select')}</Text>
                    ...
                    <View style={styles.dropdown}>
                      {UNITS.map(item => (
                        <TouchableOpacity key={item.id} style={styles.option} onPress={() => { updateVariant(index, 'unit', item.value); setShowUnit(null); }}>
                          <Text style={styles.optionText}>{t(`product_units.${item.key}`)}</Text>
                        </TouchableOpacity>
                      ))}
                    </View>
                  )}
              ...
              <View style={styles.row}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.label}>{t('update_product.mrp_required')}</Text>
                  <TextInput style={styles.input} placeholder={t('update_product.mrp_ph')} placeholderTextColor="#9CA3AF" keyboardType="decimal-pad" value={variant.mrp} onChangeText={v => updateVariant(index, 'mrp', v)} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.label}>{t('update_product.quantity_required')}</Text>
                  <TextInput style={styles.input} placeholder={t('update_product.quantity_ph')} placeholderTextColor="#9CA3AF" keyboardType="number-pad" value={variant.quantity} onChangeText={v => updateVariant(index, 'quantity', v)} />
                </View>
              </View>

              <Text style={styles.label}>{t('update_product.purchase_date_required')}</Text>
              <TouchableOpacity style={styles.dateInput} onPress={() => setOpenPurchase(index)}>
                <Icon name="calendar-outline" size={18} color={THEME} />
                <Text style={[styles.dateText, variant.purchaseDate && styles.dateTextFilled]}>{variant.purchaseDate || t('update_product.date_ph')}</Text>
              </TouchableOpacity>

              <Text style={styles.label}>{t('update_product.expiry_date')}</Text>
              <TouchableOpacity style={styles.dateInput} onPress={() => setOpenExpiry(index)}>
                <Icon name="calendar-outline" size={18} color={THEME} />
                <Text style={[styles.dateText, variant.expiryDate && styles.dateTextFilled]}>{variant.expiryDate || t('update_product.date_ph')}</Text>
              </TouchableOpacity>
        ...
        <TouchableOpacity style={[styles.saveBtn, loading && { opacity: 0.7 }]} activeOpacity={0.85} onPress={handleUpdate} disabled={loading}>
          <Icon name="checkmark-circle" size={22} color="#fff" style={{ marginRight: 8 }} />
          <Text style={styles.saveText}>{loading ? t('update_product.updating') : t('update_product.update_btn')}</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  removeImageBtn: { position: 'absolute', top: -1, right: -1, backgroundColor: '#ffffff', borderRadius: 12 },
  ...
  existingBadgeContainer: {
    ...
    backgroundColor: 'rgba(78, 143, 168, 0.85)', paddingVertical: 2, alignItems: 'center',
  },
  existingBadge: { color: '#fff', fontSize: 10, fontWeight: '700', textTransform: 'uppercase' },
