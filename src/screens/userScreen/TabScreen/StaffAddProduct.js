...
const StaffAddProduct = () => {
  ...
  const pickImageFromGallery = async () => {
    if (selectedImages.length >= 5) { showAlert({ type: 'warning', title: t('error'), message: 'Maximum 5 images allowed' }); return; }
    const image = await imagePickerService.openGallery();
    if (image) { const newImages = [...selectedImages, image]; setSelectedImages(newImages); handleChange('productImages', imagePickerService.toBase64Array(newImages)); }
  };
  ...
  const removeImage = index => { const newImages = selectedImages.filter((_, i) => i !== index); setSelectedImages(newImages); handleChange('productImages', imagePickerService.toBase64Array(newImages)); };

  const pickVideo = async () => {
    if (selectedVideos.length >= 3) { showAlert({ type: 'warning', title: t('error'), message: 'Maximum 3 videos allowed' }); return; }
    const result = await imagePickerService.openVideoGallery();
    if (result) { const newVideos = [...selectedVideos, result]; setSelectedVideos(newVideos); handleChange('productVideos', newVideos.map(v => v.uri)); }
  };
  const removeVideo = index => { const newVideos = selectedVideos.filter((_, i) => i !== index); setSelectedVideos(newVideos); handleChange('productVideos', newVideos.map(v => v.uri)); };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showAlert({ type: 'error', title: 'Error', message: 'Failed to add product' });
    } finally {
      setLoading(false);
    }
  };
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.sectionCard}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.sectionTitle}>Product Images ({selectedImages.length}/5)</Text>
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.imageScroll}>
            ...
            {selectedImages.length < 5 && (
              <TouchableOpacity style={styles.imagePickerBtn} onPress={() => setShowImagePicker(true)} activeOpacity={0.8}>
          ...
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.sectionTitle}>Product Videos ({selectedVideos.length}/3)</Text>
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.imageScroll}>
            {selectedVideos.map((vid, index) => (
              <View key={index} style={styles.videoThumb}>
                <Icon name="videocam" size={32} color={THEME} />
                <Text style={styles.videoName} numberOfLines={1}>{vid.fileName || `Video ${index + 1}`}</Text>
            ...
            {selectedVideos.length < 3 && (
              <TouchableOpacity style={styles.imagePickerBtn} onPress={pickVideo} activeOpacity={0.8}>
                <View style={styles.imagePlaceholder}>
        ...
        <View style={styles.sectionCard}>
          ...
          <TextInput style={styles.input} placeholderTextColor="#9CA3AF" placeholder={t('add_product.product_name_placeholder') || 'Enter product name'} value={form.productName} onChangeText={v => handleChange('productName', v)} />

          <Text style={styles.label}>{t('add_product.brand')} *</Text>
          <TextInput style={styles.input} placeholder={t('add_product.brand_placeholder') || 'Enter brand name'} placeholderTextColor="#9CA3AF" value={form.brand} onChangeText={v => handleChange('brand', v)} />

          <Text style={styles.label}>{t('add_product.description')}</Text>
          <TextInput style={styles.textArea} placeholder={t('add_product.description_placeholder') || 'Enter product description'} placeholderTextColor="#9CA3AF" multiline numberOfLines={4} value={form.description} onChangeText={v => handleChange('description', v)} />
        </View>
        ...
        <View style={styles.sectionCard}>
          ...
          <TouchableOpacity style={styles.select} onPress={() => setShowCategoryDropdown(!showCategoryDropdown)}>
            <Text style={[styles.selectText, form.productCategory && { color: '#1F2937' }]}>
              {form.productCategory ? CATEGORIES.find(c => c.value === form.productCategory)?.label || form.productCategory : 'Select Category'}
            </Text>
          ...
          <TextInput style={styles.input} placeholder="e.g. Wheat, Rice, Maize (comma separated)" placeholderTextColor="#9CA3AF" value={form.targetCrops} onChangeText={v => handleChange('targetCrops', v)} />
        </View>
        ...
        <View style={styles.sectionCard}>
          ...
          <TextInput style={styles.textArea} placeholder="Enter technical details" placeholderTextColor="#9CA3AF" multiline numberOfLines={3} value={form.productTechnicalDetails} onChangeText={v => handleChange('productTechnicalDetails', v)} />

          <Text style={styles.label}>How to Use</Text>
          <TextInput style={styles.textArea} placeholder="Enter usage instructions" placeholderTextColor="#9CA3AF" multiline numberOfLines={3} value={form.howToUse} onChangeText={v => handleChange('howToUse', v)} />

          <Text style={styles.label}>Product Benefits</Text>
          <TextInput style={styles.textArea} placeholder="Enter product benefits" placeholderTextColor="#9CA3AF" multiline numberOfLines={3} value={form.productBenefits} onChangeText={v => handleChange('productBenefits', v)} />
        </View>
            ...
            <View key={index} style={styles.variantCard}>
              ...
              <View style={styles.row}>
                <View style={{ flex: 1 }}>
                  ...
                  <TextInput style={styles.input} placeholder="Enter SKU" placeholderTextColor="#9CA3AF" keyboardType="numeric" value={variant.parameter} onChangeText={v => updateVariant(index, 'parameter', v)} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.label}>Unit *</Text>
                  <TouchableOpacity style={styles.select} onPress={() => setShowUnit(showUnit === index ? null : index)}>
                    <Text style={styles.selectText}>{variant.unit ? UNITS.find(u => u.value === variant.unit)?.label : 'Select'}</Text>
              ...
              <View style={styles.row}>
                <View style={{ flex: 1 }}>
                  ...
                  <TextInput style={styles.input} placeholderTextColor="#9CA3AF" placeholder="Enter MRP" keyboardType="decimal-pad" value={variant.mrp} onChangeText={v => updateVariant(index, 'mrp', v)} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.label}>Quantity *</Text>
                  <TextInput style={styles.input} placeholderTextColor="#9CA3AF" placeholder="Enter quantity" keyboardType="number-pad" value={variant.quantity} onChangeText={v => updateVariant(index, 'quantity', v)} />
                </View>
              </View>
              ...
              <TouchableOpacity style={styles.dateInput} onPress={() => setOpenPurchase(index)}>
                ...
                <Text style={[styles.dateText, variant.purchaseDate && styles.dateTextFilled]}>
              ...
              <TouchableOpacity style={styles.dateInput} onPress={() => setOpenExpiry(index)}>
                <Icon name="calendar-outline" size={18} color={THEME} />
                <Text style={[styles.dateText, variant.expiryDate && styles.dateTextFilled]}>
              ...
              <DatePicker
                ...
                date={variant.purchaseDate ? new Date(variant.purchaseDate) : new Date()}
                mode="date"
              ...
              <DatePicker
                ...
                date={variant.expiryDate ? new Date(variant.expiryDate) : new Date()}
                mode="date"
        ...
        <TouchableOpacity style={[styles.saveBtn, loading && { opacity: 0.7 }]} activeOpacity={0.85} onPress={handleSave} disabled={loading}>
          ...
          <Text style={styles.saveText}>{loading ? 'Saving...' : t('add_product.save')}</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  input: {
    ...
    paddingHorizontal: 14, marginBottom: 14, fontSize: 15, color: '#1F2937',
  ...
  textArea: {
    ...
    color: '#1F2937', backgroundColor: '#FAFAFA', marginBottom: 14,
  ...
  dropdown: {
    ...
    elevation: 6, shadowColor: '#000', shadowOpacity: 0.06,
  ...
  imageContainer: { width: 100, height: 100, marginRight: 10, position: 'relative' },
  imagePickerBtn: {
  ...
  imageText: { fontSize: 11, color: THEME, textAlign: 'center', fontWeight: '600' },
  selectedImage: { width: 100, height: 100, borderRadius: 16, resizeMode: 'cover' },
  removeBtn: { position: 'absolute', top: -6, right: -6, backgroundColor: '#ffffff', borderRadius: 12 },
  videoThumb: {
  ...
  addVariantBtn: {
    ...
    backgroundColor: '#FAF7E8', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20,
  },
  addVariantText: { fontSize: 13, color: THEME, fontWeight: '700' },
  variantCard: {
    backgroundColor: '#FAFAFA', borderRadius: 20, padding: 16,
  ...
  saveBtn: {
    ...
    shadowOpacity: 0.15, shadowRadius: 10, shadowOffset: { width: 0, height: 5 },
  },
