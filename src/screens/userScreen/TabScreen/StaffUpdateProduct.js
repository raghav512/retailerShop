...
const StaffUpdateProduct = () => {
  ...
  const [form, setForm] = useState({
    ...
    targetCrops: Array.isArray(product?.targetCrops) ? product.targetCrops.join(', ') : product?.targetCrops || '',
  ...
  const pickImageFromGallery = async () => {
    if (selectedImages.length + form.productImages.length >= 5) { showAlert({ type: 'warning', title: t('update_product.limit_reached'), message: t('update_product.max_images') }); return; }
  ...
  const removeNewImage = index => setSelectedImages(selectedImages.filter((_, i) => i !== index));
  const removeExistingImage = index => setForm(prev => ({ ...prev, productImages: prev.productImages.filter((_, i) => i !== index) }));

  /* ── Video Handlers ── */
  const pickVideo = async () => {
    if (selectedVideos.length + form.productVideos.length >= 3) { showAlert({ type: 'warning', title: t('update_product.limit_reached'), message: t('update_product.max_videos') }); return; }

    const result = await imagePickerService.openVideoGallery();
    if (result) { const newVideos = [...selectedVideos, result]; setSelectedVideos(newVideos); handleChange('productVideos', [...form.productVideos, ...newVideos.map(v => v.uri)]); }
  };
  const removeNewVideo = index => { const newVideos = selectedVideos.filter((_, i) => i !== index); setSelectedVideos(newVideos); const existingCount = (product?.productVideos || []).length; handleChange('productVideos', [...form.productVideos.slice(0, existingCount), ...newVideos.map(v => v.uri)]); };
  const removeExistingVideo = index => setForm(prev => ({ ...prev, productVideos: prev.productVideos.filter((_, i) => i !== index) }));
  ...
  const updateVariant = (index, key, value) => { const updated = [...variants]; updated[index][key] = value; setVariants(updated); };
  ...
  const handleUpdate = async () => {
    ...
    const newBase64Images = selectedImages.map(img => `data:${img.type || 'image/jpeg'};base64,${img.base64}`);
    const updatePayload = {
      productName: form.productName.trim(), brand: form.brand.trim(),
      description: form.description.trim(), productCategory: form.productCategory,
      targetCrops: cropsArray, productTechnicalDetails: form.productTechnicalDetails.trim(),
      howToUse: form.howToUse.trim(), productBenefits: form.productBenefits.trim(),
      productVideos: form.productVideos, productImages: [...existingImageUrls, ...newBase64Images],
      products: variants.map(v => { const def = { parameter: v.parameter, unit: v.unit, mrp: parseFloat(v.mrp) || 0, quantity: parseInt(v.quantity) || 0, purchaseDate: v.purchaseDate, expiryDate: v.expiryDate }; if (v._id) def._id = v._id; return def; }),
    };
    setLoading(true);
    try {
      await apiService.updateProduct(product._id, updatePayload);
      showAlert({ type: 'success', title: t('update_product.success'), message: t('update_product.update_success'), buttons: [{ text: t('send_broadcast.ok'), onPress: () => navigation.goBack() }] });
    } catch (error) {
      showAlert({ type: 'error', title: t('edit_profile.error'), message: t('update_product.update_failed') });

    } finally { setLoading(false); }
  };

  const existingVideos = form.productVideos.filter(v => typeof v === 'string' && !selectedVideos.find(sv => sv.uri === v));
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.sectionCard}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.sectionTitle}>{t('add_product.product_image')} ({selectedImages.length + form.productImages.length}/5)</Text>
          </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.imageScroll}>
            {form.productImages.map((img, index) => (
              <View key={`existing-${index}`} style={styles.imageWrapper}>
                ...
                <Text style={styles.existingBadge}>{t('update_product.saved_badge')}</Text>
              </View>
            ))}
            ...
            {(selectedImages.length + form.productImages.length) < 5 && (
              <TouchableOpacity onPress={() => setShowImagePicker(true)} style={styles.addImageBtn} activeOpacity={0.7}>
                ...
                <Text style={styles.imagePlaceholderText}>{t('update_product.add_variant')}</Text>
              </TouchableOpacity>
            )}
          </ScrollView>
          ...
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.sectionTitle}>{t('add_product.product_videos')} ({existingVideos.length + selectedVideos.length}/3)</Text>
          </View>

          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.imageScroll}>
            {existingVideos.map((uri, index) => (
              <View key={`ev-${index}`} style={styles.videoThumb}>
                <Icon name="videocam" size={32} color={THEME} />
                <Text style={styles.videoName} numberOfLines={1}>Video {index + 1}</Text>
            ...
            {selectedVideos.map((vid, index) => (
              <View key={`nv-${index}`} style={styles.videoThumb}>
                <Icon name="videocam" size={32} color={THEME} />
                <Text style={styles.videoName} numberOfLines={1}>{vid.fileName || `Video ${existingVideos.length + index + 1}`}</Text>
        ...
        <View style={styles.sectionCard}>
          ...
          <TouchableOpacity style={styles.select} onPress={() => setShowCategoryDropdown(!showCategoryDropdown)}>
            <Text style={[styles.selectText, form.productCategory && { color: '#1F2937' }]}>
              {form.productCategory ? t(`product_categories.${form.productCategory}`) : t('add_product.select_category')}
            </Text>
            ...
            <View style={styles.dropdown}>
              {CATEGORIES.map(cat => (
                <TouchableOpacity key={cat.id} style={styles.option} onPress={() => { handleChange('productCategory', cat.value); setShowCategoryDropdown(false); }}>
                  <Text style={styles.optionText}>{t(`product_categories.${cat.value}`)}</Text>
                </TouchableOpacity>
              ))}
            </View>
          )}
        ...
        <View style={styles.sectionCard}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.sectionTitle}>{t('update_product.technical_details_title')}</Text>
          </View>
          ...
          {variants.map((variant, index) => (
            <View key={index} style={styles.variantCard}>
              ...
              <View style={styles.row}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <TouchableOpacity style={styles.select} onPress={() => setShowUnit(showUnit === index ? null : index)}>
                    <Text style={styles.selectText}>{variant.unit ? t(`product_units.${variant.unit}`) : t('add_product.select')}</Text>
                    ...
                    <View style={styles.dropdown}>
                      {UNITS.map(item => (
                        <TouchableOpacity key={item.id} style={styles.option} onPress={() => { updateVariant(index, 'unit', item.value); setShowUnit(null); }}>
                          <Text style={styles.optionText}>{t(`product_units.${item.value}`)}</Text>
                        </TouchableOpacity>
                      ))}
                    </View>
                  )}
              ...
              <TouchableOpacity style={styles.dateInput} onPress={() => setOpenPurchase(index)}>
                ...
                <Text style={[styles.dateText, variant.purchaseDate && styles.dateTextFilled]}>{variant.purchaseDate || 'YYYY-MM-DD'}</Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity style={styles.dateInput} onPress={() => setOpenExpiry(index)}>
                <Icon name="calendar-outline" size={18} color={THEME} />
                <Text style={[styles.dateText, variant.expiryDate && styles.dateTextFilled]}>{variant.expiryDate || 'YYYY-MM-DD'}</Text>
              </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  existingBadge: {
    ...
    backgroundColor: 'rgba(187,169,60,0.85)', color: '#fff',
    fontSize: 9, fontWeight: '700', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 6,
  },
  ...
  videoThumb: {
    ...
    backgroundColor: '#FAF7E8', justifyContent: 'center', alignItems: 'center',
    marginRight: 10, position: 'relative', paddingHorizontal: 8,
  },
  videoName: { fontSize: 10, color: '#374151', marginTop: 4, textAlign: 'center' },

  /* VARIANTS */
  addVariantBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#FAF7E8', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20 },
