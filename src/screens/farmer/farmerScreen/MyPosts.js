    ...
    <View style={styles.card}>
      ...
      <View style={styles.bottomRow}>
        <TouchableOpacity onPress={onPickImage} style={styles.actionButton}>
          <Icon name="image-outline" size={24} color={FARMER_COLORS.primaryLight} />
  ...
  const submitEditPost = useCallback(async () => {
    ...
    try {
      ...
      if (data.status === "success") {
        ...
        showAlert({ type: 'error', title: t('error'), message: data.message || t('community_screen.load_posts_failed') });
      }
    } catch (error) {
