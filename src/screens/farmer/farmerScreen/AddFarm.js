...
import { StyleSheet, View, TextInput, TouchableOpacity, Text, PermissionsAndroid, Platform, Modal, StatusBar } from "react-native";
...
const AddFarm = ({ navigation }) => {
  ...
  const handleNext = () => {
    if (markers.length < 3) {
      showAlert({ type: 'warning', title: t('add_farm.insufficient_markers_title'), message: t('add_farm.insufficient_markers_msg') });
      return;
    }
  ...
  const handleSaveFarm = async () => {
    if (!farmName.trim()) {
      showAlert({ type: 'warning', title: t('error'), message: t('add_farm.fill_name') });
      return;
    }
    setLoading(true);
    try {
      ...
      if (!token) {
        showAlert({ type: 'warning', title: t('error'), message: t('add_farm.login_required') });
        setLoading(false);
        return;
      }
      ...
      if (!userId) {
        showAlert({ type: 'error', title: t('error'), message: t('add_farm.user_id_not_found') });
        setLoading(false);
        return;
      }
      const payload = {
      ...
      showAlert({ type: 'success', title: t('success'), message: t('add_farm.success') });
      navigation.goBack();
    } catch (error) {
      console.error('❌ Farm add error:', error.response?.data);
      showAlert({ type: 'error', title: t('error'), message: error.response?.data?.message || t('add_farm.failed') });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.searchContainer}>
        ...
        />
        <TouchableOpacity onPress={searchLocation} style={styles.searchButton} activeOpacity={0.7}>
          <Icon name="search" size={24} color="#ffffff" />
        </TouchableOpacity>
        <TouchableOpacity onPress={getCurrentLocation} style={styles.locationButton} activeOpacity={0.7}>
      ...
      >
        {currentLocation && (
          <Marker coordinate={currentLocation} title={t('add_farm.your_location')}>
      ...
      {markers.length > 0 && (
        <TouchableOpacity onPress={removeLastMarker} style={styles.undoButton} activeOpacity={0.7}>
      ...
      <View style={styles.markerInfo}>
        <Text style={styles.markerInfoText}>{t('add_farm.markers_placed', { count: markers.length })}</Text>
      </View>
      ...
      <Modal visible={showModal} transparent animationType="fade" onRequestClose={() => setShowModal(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TextInput style={styles.input} placeholder={t('add_farm.enter_farm_name')} placeholderTextColor="#9CA3AF" value={farmName} onChangeText={setFarmName} />
            <TextInput style={styles.input} placeholder={t('add_farm.farm_area')} placeholderTextColor="#9CA3AF" value={farmArea.toFixed(2)} onChangeText={(text) => setFarmArea(parseFloat(text) || 0)} keyboardType="decimal-pad" />
            <View style={styles.unitContainer}>
              <TouchableOpacity style={[styles.unitButton, unit === "acre" && styles.unitButtonActive]} onPress={() => handleUnitChange("acre")}>
                <Text style={[styles.unitText, unit === "acre" && styles.unitTextActive]}>{t('add_farm.acre')}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.unitButton, unit === "hectare" && styles.unitButtonActive]} onPress={() => handleUnitChange("hectare")}>
                <Text style={[styles.unitText, unit === "hectare" && styles.unitTextActive]}>{t('add_farm.hectare')}</Text>
              </TouchableOpacity>
            </View>
            <View style={styles.modalFooter}>
              <TouchableOpacity style={styles.nextModalButton} onPress={handleSaveFarm} disabled={loading}>
                <Text style={styles.nextModalButtonText}>{loading ? t('add_farm.saving') : t('add_farm.next')}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.cancelButton} onPress={() => setShowModal(false)}>
                <Text style={styles.cancelButtonText}>{t('add_farm.cancel')}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
