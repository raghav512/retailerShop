...
const DOCUMENT_TYPES = [
  { key: "GSTCertificate", labelKey: "gst_certificate", icon: "receipt-outline", color: "#9C27B0", maxCount: 1 },
  { key: "PANCard", labelKey: "pan_card", icon: "card-outline", color: "#F44336", maxCount: 1 },
  { key: "shopLicense", labelKey: "shop_license", icon: "storefront-outline", color: "#4CAF50", maxCount: 1 },
  { key: "tradeLicense", labelKey: "trade_license", icon: "document-text-outline", color: "#2196F3", maxCount: 1 },
  { key: "fssaiLicense", labelKey: "fssai_license", icon: "shield-checkmark-outline", color: "#FF9800", maxCount: 1 },
  { key: "bankDocuments", labelKey: "bank_documents", icon: "business-outline", color: "#607D8B", maxCount: 3 },
  { key: "identityProof", labelKey: "identity_proof", icon: "person-outline", color: "#3F51B5", maxCount: 2 },
...
const RetailerDocuments = () => {
  ...
  const fetchUploadedDocuments = useCallback(async () => {
    try {
      ...
      for (const docType of DOCUMENT_TYPES) {
        const data = await apiService.getRetailerDocuments(docType.key, userId);
        if (data.length > 0) {
  ...
  const uploadDocument = async (type) => {
    ...
    try {
      ...
      await apiService.uploadRetailerDocument(type, base64Data, doc.replaceIndex);
    ...
    showAlert({
      ...
      buttons: [
        { text: t("cancel"), style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              await apiService.deleteRetailerDocument(docTypeKey, idx);
      ...
      >
        <View style={styles.infoCard}>
          <View style={styles.infoIconBox}>
            <Icon name="information-circle" size={22} color={RETAILER_COLORS.primary} />
          </View>
...
export default RetailerDocuments;
...
const styles = StyleSheet.create({
  ...
  viewBtn: { backgroundColor: "#FFF4E6" },
