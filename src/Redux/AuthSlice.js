...
import { getAccessToken, setAccessToken, setUserData } from './Storage';
...
export const login = createAsyncThunk(
  ...
  async ({ phone, role, password }, { rejectWithValue }) => {
    try {
      ...
      const response = await api.post('/api/user/signin', requestData, {
      ...
      const normalizedRole =
        normalizeOtpRoleId(userFromApi?.role || role) ||
        role?.toString?.().trim?.() ||
        '';
...
export const logOut = createAsyncThunk('logout', async () => {
  try {
    // Call backend logout to remove FCM token before clearing local storage
    try {
      ...
      console.warn(
        '⚠️ Backend logout API failed (continuing local logout):',
        apiError.message,
      );
    }
...
export const googleLoginrout = createAsyncThunk(
  'googleLogin',
  async googleData => {
    try {
...
export const { saveRegisteredUser, clearRegisteredUser } = AuthSlice.actions;
